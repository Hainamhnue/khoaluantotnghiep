            ...
            <!-- .row -->
                <div class="row">
                    ...
                    <div class="col-md-8 col-xs-12">
                        <div class="white-box">
                            <form class="form-horizontal form-material" action="{{url('admin/profile/'.$infor->id)}}" method="POST" enctype="multipart/form-data">
                                @method('PUT')
                                {{csrf_field()}}
                                <div class="form-group">
                                    <label for="validationServer01">Tên Khoa</label>
                                    <input type="text" class="form-control is-valid" id="validationServer01" value="{{$infor->name}}" required name="name">
                                </div>
                                <div class="form-group">
                                    <label for="validationServer02">Email</label>
                                    <input type="email" class="form-control is-valid" id="validationServer02" value="{{$infor->email}}" required name="email">
                                </div>
                                <div class="form-group">
                                    <label for="validationServer03">Số điện thoại</label>
                                    <input type="number" class="form-control is-valid" id="validationServer03" placeholder="" required value="{{$infor->phone}}" name="phone">
                                </div>
                                <div class="form-group">
                                    <label for="validationServer04">Địa chỉ</label>
                                    <input type="text" class="form-control is-valid" id="validationServer04" placeholder="136 - Xuân Thủy"  value="{{$infor->address}}" name="address">
                                </div>
                                <div class="form-group">
